...
export const writeDurableJsonFile = <T>(
  ...
  const backupCount = Math.max(0, options.backupCount ?? DEFAULT_BACKUP_COUNT);
  ...
  try {
    ...
    if (options.backupCurrent !== false) {
      rotateBackups(filePath, backupCount);
    }
...
export const loadDurableJsonFile = <T>({
  ...
  onRecovery
}: DurableJsonLoadOptions<T>): DurableJsonLoadResult<T> => {
  ...
  for (const candidatePath of candidatePaths) {
    ...
    try {
      ...
      if (candidatePath !== filePath) {
        ...
        writeDurableJsonFile(filePath, value, {
      ...
      return {
        value,
        created: false,
        recoveredFrom: candidatePath !== filePath ? candidatePath : undefined
      };
    } catch {
  ...
  writeDurableJsonFile(filePath, initialValue, {
    backupCount: normalizedBackupCount,
    backupCurrent: false
  });
